"""決定論的・冪等でcollision-safeなalias割り当て。
...
parallel requests. Given that, this module is pure and deterministic.
"""
...
# alias token長はhex 12文字（48 bit）。session内10,000 mappingでもbirthday collision
# probabilityは約1.8e-10で、特定aliasの推測も現実的ではない。
# collision時はさらに延長するため、これは上限ではなく下限。
_MIN_TOKEN_HEX = 12
_MAX_TOKEN_HEX = 32
_TOKEN_STEP = 2

# hostile inputや暴走入力でmapping tableが無制限に増えないよう上限を設ける。
...
def get_or_create_alias(
    ...
    *,
    ...
    if len(session.mappings_by_fingerprint) >= MAX_MAPPINGS_PER_SESSION:
        # 無制限に増やさずfail-closedにし、messageには値を含めない。
        raise MaskingError(
...
def _allocate_alias(
    ...
    """衝突しない最短aliasを選び、衝突時はtokenを延長する。"""
