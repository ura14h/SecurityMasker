...
| Threat | Mitigation |
|---|---|
| Original secret sent to external LLM | pre-call mask + pre-send re-scan, fail-closed (§18, §26); live leakage test |
...
| Model mutates aliases (case/split/translate) | restore only exact aliases; tool-args fail-closed, no approximate restore (§24) |
...
| LiteLLM upgrade breaks hooks | pinned version + hook-contract test in CI (§37) |
| Codex/Claude Code adds unknown fields/events | unknown fields/events pass through transparently (§22, §23) |
